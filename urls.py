...
urlpatterns = [
    ...
    path('accounts/login/', auth_view. LoginView.as_view(template_name='app1/login.html', authentication_form=LoginForm), name='login'),
    path('passwordchange/',auth_view.PasswordChangeView.as_view(template_name='app1/changepassword.html',form_class=MyPasswordChangeForm,success_url='/passwordchangedone'),name='passwordchange'),
    path('passwordchangedone/',auth_view.PasswordChangeDoneView.as_view(template_name='app1/passwordchangedone.html'),name='passwordchangedone'),
    path('logout/', LogoutView.as_view(next_page='login'), name='logout'),

    path('password-reset/', auth_view.PasswordResetView.as_view(template_name='app1/password_reset.html', form_class=MyPasswordResetForm), name='password_reset'),
